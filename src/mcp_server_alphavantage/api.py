...
async def fetch_news_sentiment(
    ...
    The API finds articles that simultaneously mention ALL specified tickers.

    Args:
        tickers (Union[list[str], str]): Single ticker as string or list of tickers to search for simultaneously
            Examples: 
            - "AAPL"
            - ["COIN", "CRYPTO:BTC", "FOREX:USD"]
        datatype (str, optional): The response data type ("json" or "csv"). Defaults to "json"
        topics (str, optional): Comma-separated news topics to filter by
        ...
        limit (int, optional): Maximum number of results (1-1000). Defaults to 50

    Returns:
        Union[dict, str]: News sentiment data
    """
    # Convert tickers to string parameter
    tickers_param = tickers if isinstance(tickers, str) else ",".join(tickers)
    ...
    if topics:
        https_params["topics"] = topics
    if time_from:
        https_params["time_from"] = time_from
    if time_to:
        https_params["time_to"] = time_to
